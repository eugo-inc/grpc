...
class BuildProjectMetadata(setuptools.Command):
    ...
    def run(self):
        ...
        skip_metadata_update_on_match = os.environ.get("GRPC_PYTHON_BUILD_SKIP_METADATA_ON_VERSION_MATCH", "0")
        if skip_metadata_update_on_match == "1":
            with open(module_file_path) as module_file:
                ...
                if self._get_version_in_grpcio_metadata(module_file) == version:
                    print(f"Version match in _grpcio_metadata.py: {version}, skipping the update")
                    return
...
class BuildExt(build_ext.build_ext):
    ...
    def build_extensions(self):
        ...
        def new_compile(obj, src, ext, cc_args, extra_postargs, pp_opts):
            ...
            # @EUGO_CHANGE: @begin: use our c and c++ flags
            cpp_specific_args = {"-std=gnu++23", "-stdlib=libc++"}
            c_specific_args = {"-std=gnu17"}
            # @EUGO_CHANGE: @end
            ...
            if args_to_remove:
                extra_postargs = [arg for arg in extra_postargs if arg not in args_to_remove]
